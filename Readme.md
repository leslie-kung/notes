### 个人笔记

- [python](./python_notes)
    - [返回每个月最大天数](python_notes/demo1.md)
    - [excel和json转换](./python_notes/demo2.md)
    - [svg反爬虫](./python_notes/svg反爬虫)
    - [css反爬虫](./python_notes/css反爬虫)
    - [python版进度条](./python_notes/progress_bar.py)
    - [视频文件下载](./python_notes/video_load.py)
    
- [mysql](./mysql_notes)
    - [mysql连接池](./mysql_notes/mysql_pools.py)
    - [mysql导入导出数据](./mysql_notes/mysql导入导出数据.txt)
    - [mysql在Ubuntu下重置root密码](./mysql_notes/mysql重置root密码.txt)
    - [mysql用户创建和授权](./mysql_notes/mysql用户创建授权.md)
    - [mysql生成sql语句](./mysql_notes/generate_sql.py)

- [git](./git_notes)
    - [git命令集合](./git_notes/git.txt)
    - [git版本回退测试](./git_notes/git_test.md)

- [golang](./golang_notes)
    - [readme](./golang_notes/readme.md)

- [others](./others_notes)
    - [log初始化](./others_notes/logs.py)
    - [布隆过滤器](./others_notes/utils.py)
    - [监听模式](./others_notes/listen.py)
    - [jupyter搭建](./others_notes/jupyter搭建.md)
